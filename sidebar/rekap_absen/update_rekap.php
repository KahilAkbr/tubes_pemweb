...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["dataToUpdate"])) {
    ...
    // Loop through the dataToUpdate array to update the status in rekap_absen
    foreach ($dataToUpdate as $name => $status) {
        ...
        if (in_array($status, $statusColumns)) {
            // Get the ID of the row to be updated from the rekap_absen table
            $query = "SELECT id_jam_kerja_fix FROM rekap_absensi WHERE nama_karyawan = '$name'";
            $result = mysqli_query($conn, $query);
            $row = mysqli_fetch_assoc($result);
            $idJamKerjaFix = $row['id_jam_kerja_fix'];

            // Update the status in the rekap_absen table for the corresponding row
            $query = "UPDATE rekap_absensi SET statuss = '$status' WHERE id_jam_kerja_fix = '$idJamKerjaFix' AND nama_karyawan = '$name'";
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <main role="main" class="col-md-10 ml-sm-auto" style="background-color:whitesmoke">
                    ...
                    <h4 style="text-align:center;;margin-top:20px">Update Rekap</h4>
                    <?php if (count($rows) > 0) { ?>
                    <?php
                    ...
                    <div class="text-center">
                        <a href="rekap_absen.php" class="btn btn-dark" style="color:#FFF6E0">Kembali</a>
                        <button type="button" class="btn btn-dark" id="btnSave" style="color:#FFF6E0">Simpan</button>
                    </div>
                </main>
            </div>
        </div>
    ...
    $(document).ready(function() {
        $("#btnSave").on("click", function() {
            ...
            $.ajax({
                type: "POST",
                url: window.location
                .href, // Replace with the correct URL to your update_status.php file
                data: {
                ...
                success: function(response) {
                    ...
                    window.location.href = "rekap_absen.php";
                },
                error: function(xhr, status, error) {
                    // Handle error (if needed)
                    alert("An error occurred while updating status counts.");
                }
            });
        });
    });
